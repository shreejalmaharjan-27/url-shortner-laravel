...
<div class="flex justify-center items-center h-screen w-screen flex-col">
    <x-form action="{{ route('url.generate') }}" method="POST" class="p-4 md:p-6 bg-white rounded-lg shadow" id="urlForm">
        ...
        <div class="form-control p-4 bg-success rounded-lg mt-4 hidden" id="shortened_url_container">
            ...
            <div class="flex flex-col md:flex-row space-y-2 md:space-x-2 md:justify-between md:items-baseline">
                <input class="input input-success w-full" id="shortened_url"/>
                <button class="btn btn-md btn-secondary copy" type="button"  data-clipboard-target="#shortened_url">Copy</button>
            </div>
        </div>
    </x-form>
</div>
